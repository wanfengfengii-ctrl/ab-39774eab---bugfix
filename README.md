...
- 规模友好：按主体名建桶（封存时用零密码学解析从 DER 抽取 Name 建索引），
  ...
  走完整解析，故裁决与证据处置结果与全量解析逐字节一致。侧车仅是**派生缓存、
  不是证据**：其逐字节 SHA-256 在封存时写入受数据库保护的封存清单
  （`manifest.sidecars`），加载时先校验再使用；侧车缺失、截断、损坏、结构
  非法、版本不符或任何内容（含仅替换某条作用域值）被替换导致校验失败时，
  一律自动回退到完整解析，因此侧车被篡改**不可能**改变已封存的裁决结论；
  封存时尚无该认证的旧封存集同样走完整解析，结论保持不变。
